...
public class BJ_14502_연구소 {
	...
	// 비어있는 칸의 위치를 리스트로
	private static ArrayList<Point> empties = new ArrayList<>();
	...
	private static int infection() {
		...
		for (int i = 0; i < 3; i++) {
			copiedMap[empties.get(walls[i]).x][empties.get(walls[i]).y] = 1;
		}

		// 큐를 생성하고 바이러스를 삽입
		Queue<Point> queue = new LinkedList<>();
		for (int i = 0; i < viruses.size(); i++) {
			queue.add(new Point(viruses.get(i).x, viruses.get(i).y));
		}
		...
		while (!queue.isEmpty()) {
			...
			for (int i = 0; i < 4; i++) {
				// 빈칸 아니면 패스
				if (copiedMap[temp.x + dx[i]][temp.y + dy[i]] != 0) {
					continue;
				}
				// 빈칸이면 큐에 넣고 감염시킴
				queue.add(new Point(temp.x + dx[i], temp.y + dy[i]));
				copiedMap[temp.x + dx[i]][temp.y + dy[i]] = 2;
			}
			// 감염수 증가
			infected++;
		}
